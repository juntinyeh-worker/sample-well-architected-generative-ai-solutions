...
from .core.common.credential_utils import (
    create_aws_session,
    get_session_info,
...
@server.tool(
    name='validate_credential_configuration',
    description="""Validate AWS credential configuration and test access to AWS services.
    
    This tool verifies that AWS credentials are properly configured and tests
    access to AWS services. It's particularly useful for validating cross-account
    AssumeRole configurations before executing AWS CLI commands.
    
    The tool checks:
    - AssumeRole configuration validation
    - AWS credential access and caller identity
    - Basic AWS service connectivity
    
    Returns detailed validation results including account information and recommendations.
    """,
    annotations=ToolAnnotations(
        title='Validate AWS credentials and service access',
        readOnlyHint=True,
        openWorldHint=False,
    ),
)
async def validate_credential_configuration(
    ...
    debug: Annotated[
        bool,
        Field(description='Whether to include detailed debug information in the response'),
    ] = False,
) -> dict[str, Any] | AwsApiMcpServerErrorResponse:
    """Validate AWS credential configuration and test access to AWS services."""
    try:
        logger.info("Starting credential configuration validation")
        
        # Validate AssumeRole configuration
        assume_role_validation = validate_assume_role_config()
        ...
        try:
            ...
            aws_service_access = True
            aws_service_error = None
        except Exception as e:
            aws_service_access = False
            aws_service_error = str(e)
        
        result = {
            "credential_validation": {
                "status": "success" if "error" not in session_info else "error",
                "account_id": session_info.get("account_id"),
                "user_arn": session_info.get("arn"),
                "assume_role_configured": session_info.get("assume_role_configured", False),
            },
            "assume_role_config": assume_role_validation,
            "aws_service_access": {
                "accessible": aws_service_access,
        ...
        if not aws_service_access:
            result["recommendations"].append(
                "AWS service access failed. Ensure your IAM role/user has appropriate permissions for AWS CLI operations."
            )
        
        if assume_role_validation["configured"] and not assume_role_validation["valid"]:
            result["recommendations"].extend([
                f"AssumeRole configuration issue: {issue}" 
                for issue in assume_role_validation.get("issues", [])
            ])
        
        if assume_role_validation["configured"] and aws_service_access:
            result["recommendations"].append(
                "Cross-account AWS CLI access is properly configured and working."
            )
        ...
        if debug:
            result["debug_info"] = {
                ...
                "environment_variables": {
                    "AWS_ASSUME_ROLE_ARN": os.environ.get("AWS_ASSUME_ROLE_ARN", "Not set"),
                    "AWS_ASSUME_ROLE_SESSION_NAME": os.environ.get("AWS_ASSUME_ROLE_SESSION_NAME", "Not set"),
                    "AWS_ASSUME_ROLE_EXTERNAL_ID": "***" if os.environ.get("AWS_ASSUME_ROLE_EXTERNAL_ID") else "Not set",
                    "AWS_API_MCP_PROFILE_NAME": os.environ.get("AWS_API_MCP_PROFILE_NAME", "Not set"),
                    "AWS_REGION": os.environ.get("AWS_REGION", "Not set"),
                }
            }
        
        logger.info("Credential configuration validation completed successfully")
        return result
        
    except Exception as e:
        logger.error(f"Error during credential validation: {str(e)}")
        error_message = f"Credential validation failed: {str(e)}"
        await ctx.error(error_message)
        return AwsApiMcpServerErrorResponse(
            detail=error_message
        )
