...
class ReadOnlyOperations(dict):
    ...
    def _get_known_readonly_operations_from_metadata(self) -> dict[str, List[str]]:
        ...
        try:
            # Try to load from local package first
            with (
                importlib.resources.files(__package__.replace('.metadata', ''))
                .joinpath('data')
            ...
            # Fallback to external package if local file not found
            try:
                with (
                ...
                logger.warning(f"Could not load {METADATA_FILE}, using empty data")
                data = {}
