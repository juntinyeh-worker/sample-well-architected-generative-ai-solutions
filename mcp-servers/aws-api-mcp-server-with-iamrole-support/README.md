...
The AWS API MCP Server supports cross-account operations via AssumeRole, allowing you to execute AWS CLI commands across multiple AWS accounts securely.

#### Cross-Account Environment Variables

| Environment Variable              | Required | Default                    | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
|-----------------------------------|----------|----------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `AWS_ASSUME_ROLE_ARN`             | ❌ No     | -                          | The ARN of the IAM role to assume for cross-account access. When set, all AWS CLI commands will be executed using the assumed role credentials. Format: `arn:aws:iam::ACCOUNT-ID:role/ROLE-NAME`. If not provided, the server uses standard AWS credential chain.                                                                                                                                                                                                                                                                                                                                                           |
| `AWS_ASSUME_ROLE_SESSION_NAME`    | ❌ No     | `"aws-api-mcp-session"`    | Session name for the assumed role. Used for audit trails and CloudTrail logging. Must be 2-64 characters long and contain only alphanumeric characters, hyphens, and underscores.                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `AWS_ASSUME_ROLE_EXTERNAL_ID`     | ❌ No     | -                          | External ID for enhanced security when assuming cross-account roles. This provides an additional layer of security by requiring the external ID to match the one configured in the target role's trust policy. Recommended for production environments. Must be 2-1224 characters long.                                                                                                                                                                                                                                                                                                                                      |

#### Cross-Account Configuration Examples

**Same-Account Configuration (Standard):**
```json
{
  "mcpServers": {
    "aws-api": {
...
```json
{
  "mcpServers": {
    "aws-api-cross-account": {
      ...
      "args": ["awslabs.aws-api-mcp-server@latest"],
      "env": {
        "AWS_ASSUME_ROLE_ARN": "arn:aws:iam::123456789012:role/AWSAPIRole",
        "AWS_ASSUME_ROLE_SESSION_NAME": "aws-api-operations",
        "AWS_ASSUME_ROLE_EXTERNAL_ID": "your-secure-external-id",
        "AWS_REGION": "us-east-1"
      }
    }
  }
...
```json
{
  "mcpServers": {
    "aws-api-production": {
      ...
      "args": ["awslabs.aws-api-mcp-server@latest"],
      "env": {
        "AWS_ASSUME_ROLE_ARN": "arn:aws:iam::111111111111:role/AWSAPIRole",
        "AWS_ASSUME_ROLE_EXTERNAL_ID": "prod-external-id",
        "AWS_REGION": "us-east-1"
      }
    },
    "aws-api-staging": {
      ...
      "args": ["awslabs.aws-api-mcp-server@latest"],
      "env": {
        "AWS_ASSUME_ROLE_ARN": "arn:aws:iam::222222222222:role/AWSAPIRole",
        "AWS_ASSUME_ROLE_EXTERNAL_ID": "staging-external-id",
        "AWS_REGION": "us-east-1"
      }
    }
  }
    ...
    "Statement": [
        {
            ...
            "Resource": "arn:aws:iam::TARGET-ACCOUNT-ID:role/AWSAPIRole"
        }
    ]
}
```
    ...
    "Statement": [
        {
            "Effect": "Allow",
            "Principal": {
                "AWS": "arn:aws:iam::SOURCE-ACCOUNT-ID:role/YourSourceRole"
            },
            "Action": "sts:AssumeRole",
            "Condition": {
                "StringEquals": {
...
Use the `validate_credential_configuration` tool to verify your cross-account setup:

```
Validate my cross-account AWS credentials and service access
```
