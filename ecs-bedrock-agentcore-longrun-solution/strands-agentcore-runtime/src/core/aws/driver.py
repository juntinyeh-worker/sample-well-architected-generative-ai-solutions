...
from ..common.credential_utils import get_enhanced_credentials
...
def get_local_credentials(profile: str | None = None) -> Credentials:
    """Get the local credentials for AWS profile with enhanced AssumeRole support.
    
    This function now supports cross-account operations via AssumeRole using environment variables:
    - AWS_ASSUME_ROLE_ARN: The ARN of the role to assume (required for AssumeRole)
    - AWS_ASSUME_ROLE_SESSION_NAME: Session name for the assumed role (optional)
    - AWS_ASSUME_ROLE_EXTERNAL_ID: External ID for enhanced security (optional)
    
    Args:
        profile: Optional AWS profile name (overrides AssumeRole if specified)
        
    Returns:
        Credentials: AWS credentials object
    """
    return get_enhanced_credentials(profile)
